...
![Mauna Loa $\text{CO}_2$ structural decomposition: observed, trend, seasonal, residual](/static/img/blog-2026-07-15/fig7_co2_decomposition.png){width=100%}
...
![Zoomed view of the $\text{CO}_2$ seasonal cycles, 1990 to 1996](/static/img/blog-2026-07-15/fig8_co2_seasonal_zoom.png){width=100%}
...
The near-zero seasonal variance is the model telling us the annual $\text{CO}_2$ cycle has barely changed shape across four decades. This kind of interpretability, where the fitted noise variances are themselves findings, is something you simply do not get from a black-box smoother.

## Example 2: Filling Gaps in the AirPassengers Series {#example-airpassengers-gaps}
...
For me the practical takeaway from the two examples is that this is not just elegant theory. The $\text{CO}_2$ decomposition produced interpretable components and fitted variances that were findings in themselves, and the gap-filling beat linear interpolation by $2.5\times$ to $8\times$ on held-out truth, precisely because the model understood the seasonality. Given how much of time-series work is pre-processing, that is a tool worth having within reach rather than buried inside library internals.
...
- [`statsmodels` state-space documentation](https://www.statsmodels.org/stable/statespace.html), the library used throughout the examples.
