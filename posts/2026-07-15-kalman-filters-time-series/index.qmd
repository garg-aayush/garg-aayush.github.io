...
The mental model I keep coming back to: formulate the problem as hidden states plus dynamics plus noisy observations, run the **filter** (predict-update) when you need causal, online estimates, run the **smoother** (forward-backward) when you have the whole series offline, and let maximum likelihood (what `statsmodels` does on `fit`) pick the noise variances. Denoising, decomposition, forecasting and gap-filling all fall out of that one setup, each with calibrated uncertainty attached.
