...
- **24h AQI chart:** prefers `aqi_waqi` (CPCB stations via WAQI) when the rolling window has accumulated it, so the chart agrees with the live tile. Falls back to `aqi` (CAMS) during the 24h warm-up after rollout, or when WAQI was blocked across the entire window. The chart's caption-source label flips between "(CPCB stations · WAQI)" and "(US AQI · Open-Meteo)" based on which series is being plotted.
